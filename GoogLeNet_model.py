...
def AdaptiveFunc(z, name, activation = "relu"):
    if activation == "relu":
...
def conv_layer(name, x, filter_height, filter_width, stride, num_filters, padding='SAME', act = "relu"):
    ...
    return AdaptiveFunc(z,name = scope.name, activation=act)
...
class GoogLeNet(object):
    def __init__(self, x, keep_prob, num_classes, activation):
        self.X = x
    ...
    def _build_model(self):
        conv_1 = conv_layer('conv_1', self.X, 7, 7, 2, 64, act=self.activation)
        max_pool_1 = pool_layer('max_pool_1', conv_1, [1, 3, 3, 1], [1, 2, 2, 1])

        conv_2 = conv_layer('conv_2', max_pool_1, 3, 3, 1, 192, act=self.activation)
        conv_3 = conv_layer('conv_3', conv_2, 3, 3, 1, 192, act=self.activation)
