...
parser.add_argument('--activation_type', dest='activation_type', default='tanh', help = 'type of activation used')
...
#     from AlexNet_model_relu import AlexNet
# elif args.activation_type == "tanh":
#     from AlexNet_model_tanh import AlexNet
# elif args.activation_type == "sigmoid":
#     from AlexNet_model_sigmoid import AlexNet
# elif args.activation_type == "arelu":
#     from AlexNet_model_arelu import AlexNet
# elif args.activation_type == "arelu":
#     from AlexNet_model_arelu import AlexNet
...
def main():
    ...
    if args.model_dir[-5:] != '.ckpt':
        MODEL_DIR = args.model_dir + args.model_type + args.activation_type + '.ckpt'
        PRE_TRAINED = False
    else:
        # Train from pretrained model.
        MODEL_DIR = args.model_dir + args.model_type + args.activation_type
        PRE_TRAINED = True
    ...
    if args.model_type == 'LeNet':
        model = LeNet(x=images_holder, keep_prob=keep_prob, num_classes=10, activation = args.activation_type[:-4])
    elif args.model_type == 'AlexNet':
        model = AlexNet(x=images_holder, keep_prob=keep_prob, num_classes=10, activation = args.activation_type[:-4])
    elif args.model_type == 'VGG16':
        model = VGG16(x=images_holder, keep_prob=keep_prob, num_classes=10, activation = args.activation_type[:-4])
    elif args.model_type == 'GoogLeNet':
        model = GoogLeNet(x=images_holder, keep_prob=keep_prob, num_classes=10, activation = args.activation_type[:-4])
    elif args.model_type == 'ResNet50':
        model = ResNet50(x=images_holder, keep_prob=keep_prob, num_classes=10, activation = args.activation_type[:-4])
    else:
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(training_epochs):
            ...
            # if epoch == 399:
            # os.mkdir("./"+args.model_type+"/"+args.activation_type+"/"+str(epoch + 1))

            # saver.save(sess, "./"+args.model_type+"/"+args.activation_type+"/"+str(epoch + 1)+"/"+str(epoch + 1)+"_0.ckpt")
            # if epoch == 499:
    ...
    plt.grid()
    plt.show()
    plt.savefig("result/" + args.model_type +"/"+ args.model_type + args.activation_type)

    import pandas as pd
    df = pd.DataFrame()
    df["acc_test"] = acc_test
    df["pre_test"] = pre_test
    df["rec_test"] = rec_test
    df["f1_test"] = f1_test
    df["cross_entropy_loss"] = cross_entropy_

    df.to_csv("result/" + args.model_type +"/"+ args.model_type +args.activation_type+ "data_.csv")
