...
from tf_slim import batch_norm, flatten
# from tenso import xavier_initializer
...
def Relu(z, activation = args.activation_type[:-4]):
    if activation == "relu":
...
with tf.Session() as sess:
    ...
    saver.save(sess, "saved/DenseNet/"+args.activation_type[:-4]+"/DenseNet")
    coord.request_stop()
    coord.join(threads)
...
plt.grid()
plt.show()
plt.savefig("result/DenseNet/DENSE_NET_" + args.activation_type + ".jpg" )
...
df["f1_test"] = f1_test
df["cross_entropy_loss"] = cross_entropy

df.to_csv("result/DenseNet/DENSE_NET_" + args.activation_type + "data_.csv")
